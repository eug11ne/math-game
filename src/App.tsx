...
const [state, dispatch] = useReducer(reducer, {
  ...
  const handleAnswer = (userAnswer: string) => {
    if (userAnswer === state.answer) {
      ...
      speak(
        "Вот это да, молодец, твой юный мозг нашел правильный ответ! Красавчик!",
      );
    } else {
  ...
  const dispatch1 = (type: string, payload: any) => {
    switch (type) {
      case "RighAnswer": {
        const tempStore: AppState = {
          ...store,
          text: "Да, все правильно посчитал! СуперМегаВычислитель согласен с тобой!",
          equation: `${store.equation}=${store.answer}`,
          gif: happy,
        };
      ...
      case "NextQuestion": {
        const newQuest = initQuestion(store.level);
        const tempStore: AppState = {
          ...store,
          text: "Сколько будет?",
          equation: newQuest?.equation || "2+2",
          answer: newQuest?.answer || 4,
          gif: puzzled,
        };
      }
    }
...
const [quest, setQuest] = useState(
    initQuestion("easy") || { equation: "2+2", answer: 4 },
  );   
import puzzled from "../static/puzzled.gif";
import happy from "../static/happy.gif";
import { useReducer, useState } from "react";
import glupish from "../static/glupish-short.mp3?url";
*/